...
install_firefox_theme() {
  remove_firefox_theme
  echo "${FIREFOX_O_HOME}"*"default-esr"
  LOC=`echo "${FIREFOX_O_HOME}"*"default-esr"`
  cp -rf "${REPO_DIR}/chrome"                                      "${FIREFOX_DIR_HOME}/"*"default-esr"
  config_firefox
  echo "Copy complete."
}

config_firefox() {
    cp -rf "${REPO_DIR}/configuration"                             "${FIREFOX_DIR_HOME}/"*"default-esr"
# If LeftHandSide Tab close button is wanted
if [ "$TABSWAP" = true ] ; then
  cd "${REPO_DIR}"
	echo "- Enabling Tab close button on Left hand side"
      cp -rf "${REPO_DIR}/custom/tabs-swapclose.css"                     "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
...
if [ "$WINDOWSWAP" = true ] ; then
  cd "${REPO_DIR}"
	echo "- Enabling Tab close button on Left hand side"
      cp -rf "${REPO_DIR}/custom/windows-swapclose.css"                     "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
...
if [ "$COMPACTTAB" = true ] ; then
  cd "${REPO_DIR}"
  echo "- Enabling Tab close button on Left hand side"
      cp -rf "${REPO_DIR}/custom/compact-tabs.css"                     "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
...
if [ "$URLBAR" = true ] ; then
    ...
    cd "${REPO_DIR}"
      cp -rf "${REPO_DIR}/custom/standard-urlbar.css"              "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
...
if [ "$NOLINE" = true ] ; then
    ...
    cd "${REPO_DIR}"
      cp -rf "${REPO_DIR}/custom/noidentity.css"              "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
  echo "No identity lines configured"
fi
if [ "$TABVIEW" = true ] ; then
  cd "${REPO_DIR}"
	echo "- Re-enabling tab view button"
      cp -rf "${REPO_DIR}/custom/enabletabview.css"                     "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
...
if [ "$HIDEEXTENSION" = true ] ; then
  cd "${REPO_DIR}"
	echo "- Enabling hidden extension button"
      cp -rf "${REPO_DIR}/custom/hideextension.css"                     "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
  echo "Extension button hidden"
fi
if [ "$NOTABSINGLE" = true ] ; then
  cd "${REPO_DIR}"
	echo "- Enabling single tab minimal view"
      cp -rf "${REPO_DIR}/custom/singletabhidden.css"                     "${FIREFOX_DIR_HOME}/"*"default-esr/chrome/WhiteSur/custom"
